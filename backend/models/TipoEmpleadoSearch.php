<?php

namespace backend\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\TipoEmpleado;

/**
 * TipoEmpleadoSearch represents the model behind the search form about `backend\models\TipoEmpleado`.
 */
class TipoEmpleadoSearch extends TipoEmpleado
{
    /**
     * @inheritdoc
     */
	 

	
    public function rules()
    {
        return [
            [['id_tipo_empleado'], 'integer'],
            [['nombre_tipo_empleado', 'descripcion'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = TipoEmpleado::find();
		
        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
			
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }
		

        // grid filtering conditions
        $query->andFilterWhere([
            'id_tipo_empleado' => $this->id_tipo_empleado,
        ]);

        $query->andFilterWhere(['like', 'nombre_tipo_empleado', $this->nombre_tipo_empleado])
            ->andFilterWhere(['like', 'descripcion', $this->descripcion]);
			
        return $dataProvider;
    }
}
